...
serviceName = os.environ.get('SVC-NAME', 'd-sensor-juridic-person')
...
@app.route('/access/v1/d-sensor-juridic-person', methods=['POST'])
def svc_r_sensor_info():
    ...
    print(body)
    try:
        cursor.execute(f"DELETE FROM juridic_person_sensor WHERE sensor_id='{sensor_id}'")
        doc = {
            'timestamp': datetime.now(),
            'environment': 'b-prd',
            'text': f"DELETE FROM juridic_person_sensor WHERE sensor_id='{sensor_id}'",
            'tid': message_id,
            'serviceName': serviceName,
        }
        thread = threading.Thread(target=send_logs_es, args=(doc,))
