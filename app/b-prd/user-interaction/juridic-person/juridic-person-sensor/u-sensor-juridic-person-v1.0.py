...
serviceName = os.environ.get('SVC-NAME', 'u-sensor-juridic-person')
...
@app.route('/access/v1/u-sensor-juridic-person', methods=['POST'])
def svc_r_cam_info():
    ...
    try:
        cursor.execute(f"UPDATE juridic_person_sensor SET full_name='{full_name}', phone_numer='{phone_numer}', first_contact_name='{first_contact_name}', first_contact_phone_numer='{first_contact_phone_numer}', second_contact_name='{second_contact_name}', second_contact_phone_numer='{second_contact_phone_numer}', contract_id='{contract_id}', user_name='{user_name}', user_password='{user_password}', zip_code='{zip_code}', address_number='{address_number}' WHERE cpf='{cpf}' AND client_id='{client_id}'")
        db_connection.commit()
        doc = {
            'timestamp': datetime.now(),
            'environment': 'b-prd',
            'text': f"UPDATE juridic_person_sensor SET full_name='{full_name}', phone_numer='{phone_numer}', first_contact_name='{first_contact_name}', first_contact_phone_numer='{first_contact_phone_numer}', second_contact_name='{second_contact_name}', second_contact_phone_numer='{second_contact_phone_numer}', contract_id='{contract_id}', user_name='{user_name}', user_password='{user_password}', zip_code='{zip_code}', address_number='{address_number}' WHERE cpf='{cpf}' AND client_id='{client_id}'",
            'tid': message_id,
            'serviceName': serviceName,
        }
        thread = threading.Thread(target=send_logs_es, args=(doc,))
        thread.start()
    except psycopg2.errors.sensorViolation as e:
