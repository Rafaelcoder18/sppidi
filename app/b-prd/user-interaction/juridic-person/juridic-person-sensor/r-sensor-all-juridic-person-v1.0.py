...
serviceName = os.environ.get('SVC-NAME', 'r-sensor-all-juridic-person')
...
@app.route('/access/v1/r-sensor-all-juridic-person', methods=['POST'])
def svc_r_sensor_info():
    ...
    try:
        cursor.execute(f"SELECT * FROM juridic_person_sensor")
        try:
            client_response = cursor.fetchall()
        except:
            client_response = []
        if client_response == None:
            response_error = {"error":"clientes inexistente"}
            if 'full_name' not in body: 
                doc = {
                    'timestamp': datetime.now(),
                    'environment': 'b-prd',
                    'text': f"SELECT * FROM juridic_person_sensor",
                    'tid': message_id,
                    'serviceName': serviceName,
                }
            else:
