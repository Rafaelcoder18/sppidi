...
serviceName = os.environ.get('SVC-NAME', 'c-sensor-juridic-person')
...
@app.route('/access/v1/c-sensor-juridic-person', methods=['POST'])
def svc_r_sensor_info():
    ...
    if ('sensor_id' not in body) or ('client_id' not in body) or ('sensor_description' not in body) or ('sensor_model' not in body):   
    ...
    sensor_description = body.get('sensor_description')
    sensor_model = body.get('sensor_model')
    ...
    try:
        cursor.execute(f"INSERT INTO juridic_person_sensor VALUES ('{sensor_id}', '{client_id}', '{sensor_description}', '{sensor_model}')")
        db_connection.commit()
        doc = {
            'timestamp': datetime.now(),
            'environment': 'b-prd',
            'text': f"INSERT INTO juridic_person_sensor VALUES ('{sensor_id}', '{client_id}', '{sensor_description}', '{sensor_model}')",
            'tid': message_id,
            'serviceName': serviceName,
        }
        thread = threading.Thread(target=send_logs_es, args=(doc,))
        thread.start()
    except psycopg2.errors.juridicViolation as e:
