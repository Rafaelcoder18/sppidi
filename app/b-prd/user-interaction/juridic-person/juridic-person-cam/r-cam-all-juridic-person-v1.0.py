...
serviceName = os.environ.get('SVC-NAME', 'r-cam-all-juridic-person')
...
@app.route('/access/v1/r-cam-all-juridic-person', methods=['POST'])
def svc_r_cam_info():
    ...
    try:
        cursor.execute(f"SELECT * FROM juridic_person_cam")
        try:
        ...
        if client_response == None:
            ...
            if 'full_name' not in body: 
                doc = {
                    'timestamp': datetime.now(),
                    'environment': 'b-prd',
                    'text': f"SELECT * FROM juridic_person_cam",
                    'tid': message_id,
                    'serviceName': serviceName,
                }
            else:
