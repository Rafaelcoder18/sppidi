...
serviceName = os.environ.get('SVC-NAME', 'r-cam-juridic-person')
...
@app.route('/access/v1/r-cam-juridic-person', methods=['POST'])
def svc_r_cam_info():
    ...
    print(body)
    try:
        cursor.execute(f"SELECT * FROM juridic_person_cam WHERE cam_id='{cam_id}'")
        try:
        ...
        if client_response == None:
            ...
            doc = {
                'timestamp': datetime.now(),
                'environment': 'b-prd',
                'text': f"SELECT * FROM juridic_person_cam WHERE cam_id='{cam_id}'",
                'tid': message_id,
                'serviceName': serviceName,
            }
            thread = threading.Thread(target=send_logs_es, args=(doc,))
