...
serviceName = os.environ.get('SVC-NAME', 'c-cam-juridic-person')
...
@app.route('/access/v1/c-cam-juridic-person', methods=['POST'])
def svc_r_cam_info():
    ...
    try:
        cursor.execute(f"INSERT INTO juridic_person_cam VALUES ('{cam_id}', '{client_id}', '{cam_description}', '{cam_model}')")
        db_connection.commit()
        doc = {
            'timestamp': datetime.now(),
            'environment': 'b-prd',
            'text': f"INSERT INTO juridic_person_cam VALUES ('{cam_id}', '{client_id}', '{cam_description}', '{cam_model}')",
            'tid': message_id,
            'serviceName': serviceName,
        }
        thread = threading.Thread(target=send_logs_es, args=(doc,))
