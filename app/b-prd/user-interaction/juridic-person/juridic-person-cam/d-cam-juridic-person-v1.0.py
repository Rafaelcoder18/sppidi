...
serviceName = os.environ.get('SVC-NAME', 'd-cam-juridic-person')
...
@app.route('/access/v1/d-cam-juridic-person', methods=['POST'])
def svc_r_cam_info():
    ...
    print(body)
    try:
        cursor.execute(f"DELETE FROM juridic_person_cam WHERE cam_id='{cam_id}'")
        doc = {
            'timestamp': datetime.now(),
            'environment': 'b-prd',
            'text': f"DELETE FROM juridic_person_cam WHERE cam_id='{cam_id}'",
            'tid': message_id,
            'serviceName': serviceName,
        }
        thread = threading.Thread(target=send_logs_es, args=(doc,))
