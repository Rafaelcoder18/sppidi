...
serviceName = os.environ.get('SVC-NAME', 'r-all-juridic-person')
...
@app.route('/access/v1/r-all-juridic-person', methods=['POST'])
def svc_r_cam_info():
    ...
    try:
        cursor.execute(f"SELECT * FROM juridic_person")
        try:
        ...
        if client_response == None:
            ...
            doc = {
                'timestamp': datetime.now(),
                'environment': 'b-prd',
                'text': f"SELECT * FROM juridic_person",
                'tid': message_id,
                'serviceName': serviceName,
            }
            thread = threading.Thread(target=send_logs_es, args=(doc,))
