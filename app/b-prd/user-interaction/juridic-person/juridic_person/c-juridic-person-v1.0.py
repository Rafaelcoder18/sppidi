...
serviceName = os.environ.get('SVC-NAME', 'c-juridic-person')
...
@app.route('/access/v1/c-juridic-person', methods=['POST'])
def svc_r_cam_info():
    ...
    if ('cnpj' not in body) or ('social_reason' not in body) or ('phone_numer' not in body) or ('first_contact_name' not in body) or ('first_contact_phone_numer' not in body)  or ('client_id' not in body) or ('contract_id' not in body) or ('user_name' not in body) or ('user_password' not in body) or ('zip_code' not in body) or ('address_number' not in body):   
    ...
    phone_numer = body.get('phone_numer')
    ...
    try:
        cursor.execute(f"INSERT INTO juridic_person VALUES ('{cnpj}', '{social_reason}', '{phone_numer}', '{first_contact_name}', '{first_contact_phone_numer}', '{client_id}', '{contract_id}', '{user_name}', '{user_password}', '{zip_code}', '{address_number}')")
        db_connection.commit()
        doc = {
            'timestamp': datetime.now(),
            'environment': 'b-prd',
            'text': f"INSERT INTO juridic_person VALUES ('{cnpj}', '{social_reason}', '{phone_numer}', '{first_contact_name}', '{first_contact_phone_numer}', '{client_id}', '{contract_id}', '{user_name}', '{user_password}', '{zip_code}', '{address_number}')",
            'tid': message_id,
            'serviceName': serviceName,
        }
        thread = threading.Thread(target=send_logs_es, args=(doc,))
