...
serviceName = os.environ.get('SVC-NAME', 'c-company-manager')
...
@app.route('/access/v1/c-company-manager', methods=['POST'])
def svc_r_cam_info():
    ...
    try:
        cursor.execute(f"INSERT INTO company_manager VALUES ('{cnpj}', '{social_reason}', NOW(), '{contract_id}', '{email}', '{user_name}', '{user_password}', '{zip_code}', '{address_number}')")
        db_connection.commit()
        doc = {
            'timestamp': datetime.now(),
            'environment': 'b-prd',
            'text': f"INSERT INTO company_manager VALUES ('{cnpj}', '{social_reason}', NOW(), '{contract_id}', '{email}', '{user_name}', '{user_password}', '{zip_code}', '{address_number}')",
            'tid': message_id,
            'serviceName': serviceName,
        }
        thread = threading.Thread(target=send_logs_es, args=(doc,))
