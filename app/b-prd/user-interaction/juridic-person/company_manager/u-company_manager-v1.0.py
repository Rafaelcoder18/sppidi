...
serviceName = os.environ.get('SVC-NAME', 'u-company-manager')
...
@app.route('/access/v1/u-company-manager', methods=['POST'])
def svc_r_cam_info():
    ...
    if ('cnpj' not in body) or ('social_reason' not in body) or ('contract_id' not in body) or ('email' not in body) or ('user_name' not in body) or ('user_password' not in body) or ('zip_code' not in body) or ('address_number' not in body):   
    ...
    try:
        cursor.execute(f"UPDATE company_manager SET social_reason='{social_reason}', contract_id='{contract_id}', email='{email}', user_name='{user_name}', user_password='{user_password}', zip_code='{zip_code}', address_number='{address_number}' WHERE cnpj='{cnpj}'")
        db_connection.commit()
        doc = {
            'timestamp': datetime.now(),
            'environment': 'b-prd',
            'text': f"UPDATE company_manager SET social_reason='{social_reason}', contract_id='{contract_id}', email='{email}', user_name='{user_name}', user_password='{user_password}', zip_code='{zip_code}', address_number='{address_number}' WHERE cnpj='{cnpj}'",
            'tid': message_id,
            'serviceName': serviceName,
        }
        thread = threading.Thread(target=send_logs_es, args=(doc,))
