...
serviceName = os.environ.get('SVC-NAME', 'r-company-manager')
...
@app.route('/access/v1/r-company-manager', methods=['POST'])
def svc_r_cam_info():
    ...
    if ('cnpj' not in body) or ('user_name' not in body):   
    ...
    try:
        cursor.execute(f"SELECT * FROM company_manager WHERE cnpj='{cnpj}' AND user_name='{user_name}'")
        try:
            client_response = cursor.fetchone()
        except:
            client_response = []
        if client_response == None:
            response_error = {"error":"equipamento inexistente"}
            doc = {
                'timestamp': datetime.now(),
                'environment': 'b-prd',
                'text': f"SELECT * FROM company_manager WHERE cnpj='{cnpj}' AND user_name='{user_name}'",
                'tid': message_id,
                'serviceName': serviceName,
            }
            thread = threading.Thread(target=send_logs_es, args=(doc,))
