...
serviceName = os.environ.get('SVC-NAME', 'r-all-company-manager')
...
@app.route('/access/v1/r-all-company-manager', methods=['POST'])
def svc_r_cam_info():
    ...
    try:
        cursor.execute(f"SELECT * FROM company_manager WHERE cnpj='{cnpj}'")
        try:
        ...
        if client_response == None:
            ...
            doc = {
                'timestamp': datetime.now(),
                'environment': 'b-prd',
                'text': f"SELECT * FROM company_manager WHERE cnpj='{cnpj}'",
                'tid': message_id,
                'serviceName': serviceName,
            }
            thread = threading.Thread(target=send_logs_es, args=(doc,))
