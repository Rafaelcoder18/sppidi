...
serviceName = os.environ.get('SVC-NAME', 'r-condominium')
...
@app.route('/access/v1/r-condominium', methods=['POST'])
def svc_r_cam_info():
    ...
    try:
        if 'condominium_id' not in body: 
            cursor.execute(f"SELECT * FROM condominium WHERE client_id='{client_id}'")
        if 'client_id' not in body: 
            cursor.execute(f"SELECT * FROM condominium WHERE condominium_id='{condominium_id}'")
        try:
        ...
        if client_response == None:
            ...
            if 'condominium_id' not in body: 
                doc = {
                    'timestamp': datetime.now(),
                    'environment': 'b-prd',
                    'text': f"SELECT * FROM condominium WHERE client_id='{client_id}'",
                    'tid': message_id,
                    'serviceName': serviceName,
                }
            else:
                doc = {
                    'timestamp': datetime.now(),
                    'environment': 'b-prd',
                    'text': f"SELECT * FROM condominium WHERE condominium_id='{condominium_id}'",
                    'tid': message_id,
                    'serviceName': serviceName,
                }
            thread = threading.Thread(target=send_logs_es, args=(doc,))
