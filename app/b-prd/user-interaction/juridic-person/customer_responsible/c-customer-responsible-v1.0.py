...
serviceName = os.environ.get('SVC-NAME', 'c-customer-responsible')
...
@app.route('/access/v1/c-customer-responsible', methods=['POST'])
def svc_r_cam_info():
    ...
    try:
        cursor.execute(f"INSERT INTO customer_responsible VALUES ('{cpf}', '{full_name}', '{email}', '{user_name}', '{user_password}', '{company}')")
        db_connection.commit()
        doc = {
            'timestamp': datetime.now(),
            'environment': 'b-prd',
            'text': f"INSERT INTO customer_responsible VALUES ('{cpf}', '{full_name}', '{email}', '{user_name}', '{user_password}', '{company}')",
            'tid': message_id,
            'serviceName': serviceName,
        }
        thread = threading.Thread(target=send_logs_es, args=(doc,))
