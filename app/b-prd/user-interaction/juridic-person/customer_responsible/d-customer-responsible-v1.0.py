...
serviceName = os.environ.get('SVC-NAME', 'd-customer-responsible')
...
@app.route('/access/v1/d-customer-responsible', methods=['POST'])
def svc_r_cam_info():
    ...
    try:
        if 'full_name' not in body: 
            cursor.execute(f"DELETE FROM customer_responsible WHERE cpf='{cpf}'")
        if 'cpf' not in body: 
            cursor.execute(f"DELETE FROM customer_responsible WHERE full_name='{full_name}'")
        try:
        ...
        if client_response == None:
            ...
            if 'full_name' not in body: 
                doc = {
                    'timestamp': datetime.now(),
                    'environment': 'b-prd',
                    'text': f"DELETE FROM customer_responsible WHERE cpf='{cpf}'",
                    'tid': message_id,
                    'serviceName': serviceName,
                }
            else:
                doc = {
                    'timestamp': datetime.now(),
                    'environment': 'b-prd',
                    'text': f"DELETE FROM customer_responsible WHERE full_name='{full_name}'",
                    'tid': message_id,
                    'serviceName': serviceName,
                }
            thread = threading.Thread(target=send_logs_es, args=(doc,))
