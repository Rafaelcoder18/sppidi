...
serviceName = os.environ.get('SVC-NAME', 'r-all-customer-responsible')
...
@app.route('/access/v1/r-all-customer-responsible', methods=['POST'])
def svc_r_cam_info():
    ...
    try:
        cursor.execute(f"SELECT * FROM customer_responsible")
        try:
        ...
        if client_response == None:
            ...
            if 'full_name' not in body: 
                doc = {
                    'timestamp': datetime.now(),
                    'environment': 'b-prd',
                    'text': f"SELECT * FROM customer_responsible",
                    'tid': message_id,
                    'serviceName': serviceName,
                }
            else:
