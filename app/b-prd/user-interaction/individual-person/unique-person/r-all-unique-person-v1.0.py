...
serviceName = os.environ.get('SVC-NAME', 'r-all-unique-person')
...
@app.route('/access/v1/r-all-unique-person', methods=['POST'])
def svc_r_cam_info():
    ...
    try:
        cursor.execute(f"SELECT * FROM unique_person")
        try:
        ...
        if client_response == None:
            ...
            if 'full_name' not in body: 
                doc = {
                    'timestamp': datetime.now(),
                    'environment': 'b-prd',
                    'text': f"SELECT * FROM unique_person",
                    'tid': message_id,
                    'serviceName': serviceName,
                }
            else:
