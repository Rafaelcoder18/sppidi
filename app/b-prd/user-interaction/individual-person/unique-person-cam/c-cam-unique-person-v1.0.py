...
c_cam_unique_person_port = os.environ.get('PORT', '5003')
...
serviceName = os.environ.get('SVC-NAME', 'c-cam-unique-person')
...
@app.route('/access/v1/c-cam-unique-person', methods=['POST'])
def svc_r_cam_info():
    ...
    if ('cam_id' not in body) or ('client_id' not in body) or ('cam_description' not in body) or ('cam_model' not in body):   
    ...
    cam_description = body.get('cam_description')
    cam_model = body.get('cam_model')
    ...
    try:
        cursor.execute(f"INSERT INTO unique_person_cam VALUES ('{cam_id}', '{client_id}', '{cam_description}', '{cam_model}')")
        db_connection.commit()
        doc = {
            'timestamp': datetime.now(),
            'environment': 'b-prd',
            'text': f"INSERT INTO unique_person_cam VALUES ('{cam_id}', '{client_id}', '{cam_description}', '{cam_model}')",
            'tid': message_id,
            'serviceName': serviceName,
        }
        thread = threading.Thread(target=send_logs_es, args=(doc,))
...
if __name__ == '__main__':
    app.run(debug=True, port=c_cam_unique_person_port)
