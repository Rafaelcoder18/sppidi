...
serviceName = os.environ.get('SVC-NAME', 'r-cam-all-unique-person')
...
@app.route('/access/v1/r-cam-all-unique-person', methods=['POST'])
def svc_r_cam_info():
    ...
    try:
        cursor.execute(f"SELECT * FROM unique_person_cam")
        try:
        ...
        if client_response == None:
            ...
            if 'full_name' not in body: 
                doc = {
                    'timestamp': datetime.now(),
                    'environment': 'b-prd',
                    'text': f"SELECT * FROM unique_person_cam",
                    'tid': message_id,
                    'serviceName': serviceName,
                }
            else:
