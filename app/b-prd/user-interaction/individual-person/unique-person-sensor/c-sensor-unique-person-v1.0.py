...
serviceName = os.environ.get('SVC-NAME', 'c-sensor-unique-person')
...
@app.route('/access/v1/c-sensor-unique-person', methods=['POST'])
def svc_r_sensor_info():
    ...
    try:
        cursor.execute(f"INSERT INTO unique_person_sensor VALUES ('{sensor_id}', '{client_id}', '{sensor_description}', '{sensor_model}')")
        db_connection.commit()
        doc = {
            'timestamp': datetime.now(),
            'environment': 'b-prd',
            'text': f"INSERT INTO unique_person_sensor VALUES ('{sensor_id}', '{client_id}', '{sensor_description}', '{sensor_model}')",
            'tid': message_id,
            'serviceName': serviceName,
        }
        thread = threading.Thread(target=send_logs_es, args=(doc,))
