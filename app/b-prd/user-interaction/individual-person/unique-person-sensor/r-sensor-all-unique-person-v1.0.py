...
serviceName = os.environ.get('SVC-NAME', 'r-sensor-all-unique-person')
...
@app.route('/access/v1/r-sensor-all-unique-person', methods=['POST'])
def svc_r_sensor_info():
    ...
    try:
        cursor.execute(f"SELECT * FROM unique_person_sensor")
        try:
        ...
        if client_response == None:
            ...
            if 'full_name' not in body: 
                doc = {
                    'timestamp': datetime.now(),
                    'environment': 'b-prd',
                    'text': f"SELECT * FROM unique_person_sensor",
                    'tid': message_id,
                    'serviceName': serviceName,
                }
            else:
